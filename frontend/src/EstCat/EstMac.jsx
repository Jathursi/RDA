...
function EstMac({ details, setDetails }) {
    const createDetailsHandler = (detailsState, setDetailsState, defaultValues) => {
        const handleChange = (event, index) => {
            const { name, value } = event.target;
            const newData = [...detailsState];
    ...
    const macHandler = createDetailsHandler(details, setDetails, { Machining: '', Mac_cost: '', MacQ: '' });

    return (
        <form className='mt-4'>
            <h3>Machining Details</h3>
            {details.map((mac, index) => (
                <div key={index}>
                    <div className='row mb-3'>
                        <label className='col-sm-2 col-form-label'>Machining:</label>
                        <div className='col-sm-10'>
            ...
            <div className='d-grid gap-3'>
                <button className='btn btn-secondary' type="button" onClick={macHandler.handleAdd}>Add Machining</button>
            </div>
        </form>
    );
}
