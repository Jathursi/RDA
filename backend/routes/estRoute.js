...
// //     try {
// //         // Check if the EstID exists in the supplier table
// //         const supplier = await Supplier.findOne({ where: { EstID } });
// //         if (!supplier) {
// //             return res.status(400).json({ error: 'Invalid EstID. Supplier does not exist.' });
// //         }

// //         // Save category details
// //         const categoryDetails = JSON.parse(details).map((item) => ({
// //             ...item,
// //             EstID, // Ensure supID is included
// //         }));
...
//     const { logbookID } = req.params;
//     try {
//         // Fetch all estimates for the logbookID
//         const estimates = await Estimate.findAll({ where: { logbookID } });

//         // Fetch images for each estimate based on EstID
//         const images = await Promise.all(
//             estimates.map(async (estimate) => {
//                 const estimateImages = await EstImage.findAll({ where: { EstID: estimate.id } });
//                 return estimateImages.map(image => ({
...
//         res.status(200).json(formattedImages);  // Send the images with EstID
//     } catch (error) {
//         console.error('Error fetching images:', error);
//         res.status(500).json({ error: 'Failed to fetch images' });
//     }
